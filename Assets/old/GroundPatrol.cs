...
public class GroundPatrol : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit2D WallInfo = Physics2D.Raycast(groundDetect.position, Vector2.left, 0.001f);

        if (GroundInfo.collider == false || WallInfo.collider == true) //колайдер для луча вниз платформы для поворотов
        {
